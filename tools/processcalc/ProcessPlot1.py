...
def plot_cpu_memory_usage(file):
    ...
    grouped = df.groupby([CSV_COL_PROCESSID_INDEX, CSV_COL_PROCESSCMD_INDEX])
    ...
    for (pid, name), group in grouped:
        if not((name in process_to_ignore) or any(name.startswith(prefix) for prefix in prefix_to_ignore)):
            ...
            new_max_times = 0
            if max_times == -1:
                new_max_times = max(group[CSV_COL_TIMES_INDEX]) + 1
            else:
                new_max_times = max(max_times, max(group[CSV_COL_TIMES_INDEX])) + 1
            ...
            for index, row in group.iterrows():
                item_cpu = row[CSV_COL_CPU_INDEX]
                item_memory = row[CSV_COL_MEMORY_INDEX]
                item_time = row[CSV_COL_TIMES_INDEX]
    ...
    # Plot CPU usage
    plt.subplot(2, 1, 1)
    plt.plot(list(range(0, 225)), cpu_data[0][:225])
    plt.xlabel('Time (s)')
    plt.ylabel('CPU Usage (%)')
    plt.title(f'Global CPU Usage Over Time')

    plt.axvline(x=launching_time, color='gray', linestyle='--')
    plt.text(int(launching_time / 2), int(max_cpu), 'Launching\nsimulator', horizontalalignment='center', verticalalignment='top', color='gray')
    plt.axvline(x=init_step_time, color='gray', linestyle='--')
    plt.text(int((init_step_time + launching_time)/2), int(max_cpu), 'Initialization', horizontalalignment='center', verticalalignment='top', color='gray')
    plt.axvline(x=first_step_time, color='gray', linestyle='--')
    plt.text(int((first_step_time + init_step_time)/2), int(max_cpu), 'First Step', horizontalalignment='right', verticalalignment='center', color='gray')
    ...
    # Plot Memory usage
    plt.subplot(2, 1, 2)
    plt.plot(list(range(0, 225)), memory_data[0][:225], color='orange')
    plt.xlabel('Time (s)')
    ...
    plt.axvline(x=init_step_time, color='gray', linestyle='--')
    plt.text(int((init_step_time + launching_time)/2), int(max_memory), 'Initialization', horizontalalignment='center', verticalalignment='top', color='gray')
    plt.axvline(x=first_step_time, color='gray', linestyle='--')
    plt.text(int((first_step_time + init_step_time)/2), int(max_memory), 'First Step', horizontalalignment='right', verticalalignment='center', color='gray')
...
if __name__ == "__main__":
    plot_cpu_memory_usage('../../reinforcement/tmp/cpu_memory_process_usage_single_step.csv')
